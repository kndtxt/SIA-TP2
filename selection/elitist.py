...
def selection_elitist(geneticAlgorithm: GeneticAlgorithm, quantity: int) -> List[Individual]:
        """Selección por Elitista."""
        #geneticAlgorithm.calculate_population_fitness(population)
        pop_size = len(geneticAlgorithm.population)
        geneticAlgorithm.population.sort(key=lambda ind: ind.fitness, reverse=True) #de mejor a peor
